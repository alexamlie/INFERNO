...
if [ $# == 13 ]; then
	...
	${METAXCAN_DIR}/MetaMany.py --gwas_file ${SUMMARY_STATF} --snp_column ${RSID_NAME} \
	    ...
	    --position_column ${POS_NAME} --beta_column ${BETA_NAME} --freq_column ${MAF_NAME} \
	...
	awk -F$'\t' -v RSID=${RSID_COL} -v POS=${POS_COL} -v PVAL=${PVAL_COL} \
	    ...
	    -v BETA=${BETA_COL} -v NUM_COLS=${NUM_COLS} \
	...
	${METAXCAN_DIR}/MetaMany.py --gwas_file ${OUTDIR}/summary_stats.header_added.txt \
	    ...
	    "beta" --freq_column "MAF" --separator $'\t' \
    ...
    echo "Usage: $0 <metaXcan dir> <output directory> <summary stat file> <GTEx v7 PredictDB directory> <rsid column num> <position column num> <p-value column num> <chromsoome column num> <allele 1 column num> <allele 2 column num> <MAF column num> <beta column num> <true/false for summary stat header>"
fi
