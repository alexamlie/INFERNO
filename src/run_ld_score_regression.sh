...
if [ $# == 17 ]; then
	...
	if [ ${USE_BETA} == "True" ]; then
		...
		--signed-sumstats "${EFFECT_NAME},0" --N ${N}
	else
	    python ./munge_sumstats.py --sumstats ${SUMMARY_STATF} --merge-alleles ${MUNGE_SNPLIST} \
		--out ${OUTDIR}/${OUTPREFIX}.munged_data --snp "${RSID_NAME}" --a1 "${ALLELE1_NAME}" \
		--a2 "${ALLELE2_NAME}" --p "${PVAL_NAME}" --frq "${MAF_NAME}" \
		--signed-sumstats "${EFFECT_NAME},1" --N ${N}
	fi
	...
	python ./ldsc.py --h2 ${OUTDIR}/${OUTPREFIX}.munged_data.sumstats.gz --ref-ld-chr \
	 ...
	 awk -F$'\t' -v RSID=${RSID_COL} -v PVAL=${PVAL_COL} -v A1=${ALLELE1_COL} \
	     -v A2=${ALLELE2_COL} -v MAF=${MAF_COL} -v EFFECT=${EFFECT_COL} -v NUM_COLS=${NUM_COLS} \
	 ...
	 if [ ${USE_BETA} == "True" ]; then
		 ...
		 --a2 "allele2" --p "pval" --frq "MAF" --signed-sumstats "effect,0"
	 else
	     python ./munge_sumstats.py --sumstats ${OUTDIR}/${OUTPREFIX}.summary_stats.header_added.txt \
		 --merge-alleles ${MUNGE_SNPLIST} --N ${N} \
		 --out ${OUTDIR}/${OUTPREFIX}.munged_data --snp "rsID" --a1 "allele1" \
		 --a2 "allele2" --p "pval" --frq "MAF" --signed-sumstats "effect,1"
	 fi	     
	...
	if [ ${USE_BETA} == "True" ]; then
		...
		--out ${OUTDIR}/${OUTPREFIX}.munged_data --snp "${RSID_NAME}" --a1 "${ALLELE1_NAME}" \
		--a2 "${ALLELE2_NAME}" --p "${PVAL_NAME}" --frq "${MAF_NAME}" \
		--signed-sumstats "${EFFECT_NAME},0" --N ${N} 2>&1 | tee -a ${LOGFILE}
	else
	    python ./munge_sumstats.py --sumstats ${SUMMARY_STATF} --merge-alleles ${MUNGE_SNPLIST} \
		--out ${OUTDIR}/${OUTPREFIX}.munged_data --snp "${RSID_NAME}" --a1 "${ALLELE1_NAME}" \
		--a2 "${ALLELE2_NAME}" --p "${PVAL_NAME}" --frq "${MAF_NAME}" \
		--signed-sumstats "${EFFECT_NAME},1" --N ${N} 2>&1 | tee -a ${LOGFILE}
	fi
	 ...
	 if [ ${USE_BETA} == "True" ]; then
		 ...
		 --merge-alleles ${MUNGE_SNPLIST} --N ${N} \
		 --out ${OUTDIR}/${OUTPREFIX}.munged_data --snp "rsID" --a1 "allele1" \
		 --a2 "allele2" --p "pval" --frq "MAF" --signed-sumstats "effect,0" 2>&1 | tee ${LOGFILE}
	 else
	     python ./munge_sumstats.py --sumstats ${OUTDIR}/${OUTPREFIX}.summary_stats.header_added.txt \
		 --merge-alleles ${MUNGE_SNPLIST} --N ${N} \
		 --out ${OUTDIR}/${OUTPREFIX}.munged_data --snp "rsID" --a1 "allele1" \
		 --a2 "allele2" --p "pval" --frq "MAF" --signed-sumstats "effect,1" 2>&1 | tee ${LOGFILE}
	 fi	     
