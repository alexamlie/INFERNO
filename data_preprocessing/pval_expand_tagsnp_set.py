...
def expand_tagsnp_list_consistent_effects(input_snp_list, summary_stats_file, output_file, output_allele_info, rsid_col, pos_col, chr_col, pval_col, beta_col, maf_col, sig_multiplier=None, locus_thresh=None, dist_threshold=500000):
    """   
    this method uses a single summary statistics file as input, and that
    file must include beta effect values. this is to make sure that any
    expanded variants are in the same direction as the tag SNPs    
    """
    ## create the output directory, if needed
    try:
        os.makedirs(os.path.dirname(output_file))
            ...
            for line in summary_stats:
                ...
                if chr_orig in tag_snp_dict.keys():
                    for tagsnp in tag_snp_dict[chr_orig]:
                        ...
                        this_snp_pos = int(this_snp_data[pos_col])
                        if (abs(int(tagsnp[2])-this_snp_pos) <= dist_threshold and tagsnp[0]!=this_snp_data[rsid_col]):
                            this_snp_effect = float(this_snp_data[pval_col])
                            if float(this_snp_data[maf_col]) > 0.50:
...
if __name__=="__main__":
    ...
    parser.add_argument("--beta_col", type=int, help="The column number containing SNP beta values") 
    parser.add_argument("--maf_col", type=int, help="The column number containing SNP MAF values")
