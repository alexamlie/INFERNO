...
if [[ "$#" -lt 5 ]]; then
	...
	echo "Number of SNPs overlapping enhancer by midpoint: ${ENH_SNPS} (${ENH_PROP} of total)" | tee -a ${OUTDIR}/${PREFIX}_annotation_overlap_summary.txt
    fi
	...
	ENH_PROP=`echo "scale=8; ${ENH_SNPS} / ${UNIQ_SNPS}" | bc`
	echo "Number of SNPs overlapping extended enhancer locus: ${ENH_SNPS} (${ENH_PROP} of total)" | tee -a ${OUTDIR}/${PREFIX}_annotation_overlap_summary.txt
    fi
    ...
    echo "Number of SNPs that are eQTLs by direct overlap: ${EQTL_SNPS} (${EQTL_PROP} of total)" | tee -a ${OUTDIR}/${PREFIX}_annotation_overlap_summary.txt
    ...
    echo "Number of SNPs that overlap a roadmap chromHMM enhancer state: ${HMM_SNPS} (${HMM_PROP} of total)" | tee -a ${OUTDIR}/${PREFIX}_annotation_overlap_summary.txt
	...
	echo "Number of SNPs overlapping FactorBook TFBSs: ${TFBS_SNPS} (${TFBS_PROP} of total)" | tee -a ${OUTDIR}/${PREFIX}_annotation_overlap_summary.txt
    fi
	...
	TFBS_PROP=`echo "scale=8; ${TFBS_SNPS} / ${UNIQ_SNPS}" | bc`
	echo "Number of SNPs overlapping HOMER-predicted TFBSs: ${TFBS_SNPS} (${TFBS_PROP} of total)" | tee -a ${OUTDIR}/${PREFIX}_annotation_overlap_summary.txt
